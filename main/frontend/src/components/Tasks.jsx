...
const Tasks = () => {
  ...
  const formatDate = (dateString) => {
    ...
    return dateWithoutTime.split('-').reverse().join('-'); // Convert from yyyy-mm-dd to dd-mm-yyyy
  };
            ...
            {tasks.length === 0 ? (
              ...
              tasks.map((task, index) => (
                <div key={task._id} className='bg-white my-4 p-4 text-gray-600 rounded-md shadow-md'>
                  ...
                  <div className='whitespace-pre'>{formatDate(task.duedate)}</div> {/* Display formatted date */}
